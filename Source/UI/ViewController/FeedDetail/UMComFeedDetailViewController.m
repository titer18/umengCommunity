...
@property (nonatomic, strong) NSMutableArray *reloadCommentsDict;
...
@property (strong, nonatomic) UILabel *likeStatusLabel;
         ...
         viewExtra:(NSDictionary *)viewExtra
{
    ...
    if (self) {
        ...
        self.commentId = [viewExtra valueForKey:@"comment_id"];
...
- (void)viewDidLoad
{
    ...
    self.reloadCommentsDict = [NSMutableArray array];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if (section == 0) {
        ...
        count = self.reloadCommentsDict.count;
        if (count > 0) {
            if (count >= 20) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        NSDictionary *dict = self.reloadCommentsDict[indexPath.row];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        if (indexPath.row < self.reloadCommentsDict.count && indexPath.row < self.reloadCommentsDict.count) {
            UMComMutiText *mutiText = [self.reloadCommentsDict[indexPath.row] valueForKey:kUMComCommentMutitext];
...
- (void)handleCoreDataDataWithData:(NSArray *)data error:(NSError *)error dataHandleFinish:(DataHandleFinish)finishHandler
{
    if (!error && [data isKindOfClass:[NSArray class]]) {
        [self.reloadCommentsDict removeAllObjects];
...
- (void)handleLoadMoreDataWithData:(NSArray *)data error:(NSError *)error dataHandleFinish:(DataHandleFinish)finishHandler
{
    if (!error && [data isKindOfClass:[NSArray class]]) {
        ...
        __weak typeof(self) weakSelf = self;
        [self reloadCommentTableViewArrWithComments:tempArray completion:^{
            int commentCount = [weakSelf.feed.comments_count intValue];
            if (commentCount < tempArray.count) {
                commentCount = (int)tempArray.count;
            }
...
- (void)reloadViewsWithFeed
{
    ...
    if (self.tableView.contentSize.height < comtentSizeHeight && self.reloadCommentsDict.count > 0) {
    ...
    [self loadAllData:nil fromServer:^(NSArray *data, BOOL haveNextPage, NSError *error) {
        ...
        if (weakSelf.showType == UMComShowFromClickComment || weakSelf.showType == UMComShowFromClickRemoteNotice) {
            if (isViewDidAppear == YES && weakSelf.showType == UMComShowFromClickComment) {
                if (weakSelf.reloadCommentsDict.count > 0) {
...
- (void)reloadCommentTableViewArrWithComments:(NSArray *)reloadComments completion:(void (^)())completion
{
    for (UMComComment *comment in reloadComments) {
        ...
        [self insertComment:comment isAdd:NO];
...
- (void)didClickOnForward:(UIButton *)sender {
    [[UMComAction action] performActionAfterLogin:self.feed viewController:self completion:^(id responseObject, NSError *error) {
        if (!error) {
...
- (void)onClickHandlButton:(UIButton *)sender
{
    ...
    [[UMComAction action] performActionAfterLogin:nil viewController:self completion:^(id responseObject, NSError *error) {
        if (!error) {
            ...
            //当前feed的用户和登陆用户是一样的，不需要举报
            if (([[UMComSession sharedInstance].loginUser.uid isEqualToString:self.feed.creator.uid])) {
                //直接删除上面的加入的举报字段
...
- (void)deleteComment
{
    ...
    [UMComPushRequest deleteWithComment:self.selectedComment feed:self.feed completion:^(id responseObject, NSError *error) {
        if (!error) {
            ...
            [self refreshFeedsComments:weakSelf.feed.feedID block:nil];
            if (self.feedOperationFinishDelegate && [self.feedOperationFinishDelegate respondsToSelector:@selector(reloadDataWhenFeedOperationFinish:)]) {
                [self.feedOperationFinishDelegate reloadDataWhenFeedOperationFinish:weakSelf.feed];
            }
        }
        [self reloadViewsWithFeed];
    }];
...
- (void)postComment:(NSString *)content
{
    ...
    [UMComPushRequest commentFeedWithFeed:self.feed
        ...
        if (error) {
            ...
            [self insertComment:responseObject isAdd:YES];
            [[NSNotificationCenter defaultCenter] postNotificationName:kUMComCommentOperationFinishNotification object:weakSelf.feed];
        }
          if (weakSelf.feedOperationFinishDelegate && [weakSelf.feedOperationFinishDelegate respondsToSelector:@selector(reloadDataWhenFeedOperationFinish:)]) {
...
- (void)insertComment:(UMComComment *)comment isAdd:(BOOL)isAdd
{
    ...
    if (isAdd == NO) {
        [self.reloadCommentsDict addObject:commentDict];
        index = self.reloadCommentsDict.count - 1;
    }else{
        [self.reloadCommentsDict insertObject:commentDict atIndex:0];
    }
