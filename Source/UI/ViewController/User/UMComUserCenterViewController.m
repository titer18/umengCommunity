...
//    feedTableViewController.tableView.tableHeaderView = [[UIView alloc]initWithFrame:self.headerView.frame];
    ...
    followersTableViewController.headerViewHeight = 60;
    ...
    fanTableViewController.headerViewHeight = 60;
...
{
    if (scrollView.contentOffset.y >0 && scrollView.contentOffset.y > lastPosition.y+30) {
        [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseIn animations:^{
            [self subViewControllersScrollToTop];
        } completion:nil];
    }else{
        if (scrollView.contentOffset.y < lastPosition.y-15) {
            [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseIn animations:^{
                [self subViewControllersScrollToBottom];
...
- (void)subViewControllersScrollToTop
{
    ...
    if (self.headerView.frame.origin.y == 0) {
        for (UIViewController *viewController in self.childViewControllers) {
            ...
            headFrame.origin.y = - self.headerView.frame.size.height + self.menuView.frame.size.height;
            self.headerView.frame = headFrame;
            frame.origin.y = self.menuView.frame.size.height;
            frame.size.height = self.view.frame.size.height - frame.origin.y;
            viewController.view.frame = frame;
        }
    }
...
- (void)subViewControllersScrollToBottom
{
    if (self.headerView.frame.origin.y < 0) {
        for (UIViewController *viewController in self.childViewControllers) {
            CGRect frame = viewController.view.frame;
