...
- (void)displayWithUser:(UMComUser *)user
{
    ...
    NSNumber *feed_count = user.feed_count;
    ...
    self.descriptionLable.text =  [NSString stringWithFormat:@"发表消息: %@ / 粉丝: %@",feed_count,fans_count];
...
- (void)changeFocusWithIsFollow:(BOOL)isFollow
{
    ...
    self.descriptionLable.text =  [NSString stringWithFormat:@"发表消息: %@ / 粉丝: %@",[self.user feed_count],self.user.fans_count];
}
