...
- (void)viewDidLoad
{
    ...
    if ([UMComSession sharedInstance].draftFeed) {
        ...
        if (self.editFeedEntity.images.count > 0) {
...
- (void)showWhenEditNewFeed
{
    ...
    [self handleOriginImages:self.originImages];
}
...
- (void)handleOriginImages:(NSArray *)images{
    ...
    if (!self.imagesBgView) {
        ...
        self.addedImageView.imagesDeleteFinish = ^(NSInteger index){
            if (index < weakSelf.originImages.count) {
...
- (void)viewsFrameChange
{
    ...
    CGRect addImageViewFrame = self.addedImageView.frame;
    ...
    if (self.forwardFeed) {
        ...
        addImageViewFrame.origin.y = realTextViewFrame.size.height + realTextViewFrame.origin.y;
        self.imagesBgView.frame = addImageViewFrame;
        self.realTextView.frame = realTextViewFrame;
                         ...
                         response:(void (^)(id responseObject,NSError *error))response
{
    ...
    if ([self isPermission_bulletin]) {//是否具有发公告的权限
