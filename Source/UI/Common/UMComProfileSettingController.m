...
-(void)viewDidLoad
{
    ...
    int gender = [self.userAccount.gender intValue];
...
//    self.nameField.text = self.userAccount.name;
//    [self setGender:[self.userAccount.gender integerValue]];
//    [self.userPortrait setImageURL:self.userAccount.icon_url placeHolderImage:[UMComImageView placeHolderImageGender:self.userAccount.gender.integerValue]];
...
- (void)setUserProfile
{
    ...
    [self.userPortrait  setImageURL:self.userAccount.icon_url placeHolderImage:nil];
    [self setGender:self.userAccount.gender.integerValue];
    
    if ([self.userAccount.gender intValue] == 0) {
        [self.genderButton setTitle:UMComLocalizedString(@"Female",@"女") forState:UIControlStateNormal];
    } else {
        [self.genderButton setTitle:UMComLocalizedString(@"Male",@"男") forState:UIControlStateNormal];
    }
...
-(void)onClickSave
{
    ...
    if (self.settingCompletion) {
        ...
        if ([UMComSession sharedInstance].isLogin == YES) {
            [UMComPushRequest updateWithUser:self.userAccount completion:^(NSError *error) {
                if (!error) {
